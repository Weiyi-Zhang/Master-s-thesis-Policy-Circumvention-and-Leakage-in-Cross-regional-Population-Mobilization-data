#--------------------
#Objective: 毕业论文数据处理与分析-arrival and no cooperation
...
raw_anc <- read.csv("leakage-arrival and no cooperation experiment-arrival and no cooperation-spreadsheet.csv")
##------------------------

##--Formatting--
# time_option: 3, 7, 14, 21 4*350=1400
# anc
combinations <- raw_anc[c(2,3),]
...
anc <- raw_anc[c(11:nrow(raw_anc)),]
rownames(anc) <- 1:nrow(anc)
anc <- select(anc, -1)
anc[1,] <- gsub(pattern = "compute-", replacement = "", anc[1,])
anc_3days <- select(anc, 1:3150)
anc_7days <- select(anc, 3151:6300)
anc_14days <- select(anc, 6301:9450)
anc_21days <- select(anc, 9451:12600)
for (i in 1:ncol(combinations)) {
  for (j in 1:10) {
    for (w in 1:9) {
      colnames(anc_3days)[(i-1)*90+(j-1)*9+w] <- paste(anc_3days[1,(i-1)*90+(j-1)*9+w], j, combinations[3,i], sep = "_")
      colnames(anc_7days)[(i-1)*90+(j-1)*9+w] <- paste(anc_7days[1,(i-1)*90+(j-1)*9+w], j, combinations[3,i], sep = "_")
      colnames(anc_14days)[(i-1)*90+(j-1)*9+w] <- paste(anc_14days[1,(i-1)*90+(j-1)*9+w], j, combinations[3,i], sep = "_")
      colnames(anc_21days)[(i-1)*90+(j-1)*9+w] <- paste(anc_21days[1,(i-1)*90+(j-1)*9+w], j, combinations[3,i], sep = "_")
    }
  }
}
anc_3days <- anc_3days[-c(1,2),]
anc_7days <- anc_7days[-c(1,2),]
anc_14days <- anc_14days[-c(1,2),]
anc_21days <- anc_21days[-c(1,2),]
rownames(anc_3days) <- 1:nrow(anc_3days)
rownames(anc_7days) <- 1:nrow(anc_7days)
rownames(anc_14days) <- 1:nrow(anc_14days)
rownames(anc_21days) <- 1:nrow(anc_21days)
for (i in 1:ncol(anc_3days)) {
  anc_3days[,i] <- as.numeric(anc_3days[,i])
  anc_7days[,i] <- as.numeric(anc_7days[,i])
  anc_14days[,i] <- as.numeric(anc_14days[,i])
  anc_21days[,i] <- as.numeric(anc_21days[,i])
}
##------------------------
...
# anc: R, M
# 3days
anc_3days_for_R <- anc_3days %>%
  select(seq(1, 3142, by = 9))
anc_3days_for_M <- anc_3days %>%
  select(seq(2, 3143, by = 9))

# create blank frames to store variances of variables
variance_3days_R <- data.frame(matrix(NA, nrow = nrow(anc_3days_for_R), ncol = 35))
variance_3days_M <- data.frame(matrix(NA, nrow = nrow(anc_3days_for_M), ncol = 35))
# computing variances of repeated groups and rename the columns
for (i in 1:35) {
  # R: Risk
  testgroup_R <- anc_3days_for_R %>%
...
# 7days
anc_7days_for_R <- anc_7days %>%
  select(seq(1, 3142, by = 9))
anc_7days_for_M <- anc_7days %>%
  select(seq(2, 3143, by = 9))

# create blank frames to store variances of variables
variance_7days_R <- data.frame(matrix(NA, nrow = nrow(anc_7days_for_R), ncol = 35))
variance_7days_M <- data.frame(matrix(NA, nrow = nrow(anc_7days_for_M), ncol = 35))
# computing variances of repeated groups and rename the columns
for (i in 1:35) {
  # R: Risk
  testgroup_R <- anc_7days_for_R %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  testgroup_R <- as.matrix(testgroup_R)
  variance_7days_R[,i] <- rowSds(testgroup_R)
  colnames(variance_7days_R)[i] <- paste("Rsd", combinations[3,i], sep = "_")
  # M: Mobility
  testgroup_M <- anc_7days_for_M %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  testgroup_M <- as.matrix(testgroup_M)
...
# 14days
anc_14days_for_R <- anc_14days %>%
  select(seq(1, 3142, by = 9))
anc_14days_for_M <- anc_14days %>%
  select(seq(2, 3143, by = 9))

# create blank frames to store variances of variables
variance_14days_R <- data.frame(matrix(NA, nrow = nrow(anc_14days_for_R), ncol = 35))
variance_14days_M <- data.frame(matrix(NA, nrow = nrow(anc_14days_for_M), ncol = 35))
# computing variances of repeated groups and rename the columns
for (i in 1:35) {
  # R: Risk
  testgroup_R <- anc_14days_for_R %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  testgroup_R <- as.matrix(testgroup_R)
  variance_14days_R[,i] <- rowSds(testgroup_R)
  colnames(variance_14days_R)[i] <- paste("Rsd", combinations[3,i], sep = "_")
  # M: Mobility
  testgroup_M <- anc_14days_for_M %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  testgroup_M <- as.matrix(testgroup_M)
...
# 21days
anc_21days_for_R <- anc_21days %>%
  select(seq(1, 3142, by = 9))
anc_21days_for_M <- anc_21days %>%
  select(seq(2, 3143, by = 9))

# create blank frames to store variances of variables
variance_21days_R <- data.frame(matrix(NA, nrow = nrow(anc_21days_for_R), ncol = 35))
variance_21days_M <- data.frame(matrix(NA, nrow = nrow(anc_21days_for_M), ncol = 35))
# computing variances of repeated groups and rename the columns
for (i in 1:35) {
  # R: Risk
  testgroup_R <- anc_21days_for_R %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  testgroup_R <- as.matrix(testgroup_R)
  variance_21days_R[,i] <- rowSds(testgroup_R)
  colnames(variance_21days_R)[i] <- paste("Rsd", combinations[3,i], sep = "_")
  # M: Mobility
  testgroup_M <- anc_21days_for_M %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  testgroup_M <- as.matrix(testgroup_M)
...
# 利用ggarrange将图像排列在一起
plot_variance_anc <- ggarrange(plot_variance_3days_R, plot_variance_7days_R, plot_variance_14days_R, plot_variance_21days_R,
                               plot_variance_3days_M, plot_variance_7days_M, plot_variance_14days_M, plot_variance_21days_M,
                               ncol = 4,nrow =2)
plot_variance_anc # 手动export大小合适的图
...
##--Discrete integral--
# anc: R, M, tt(transit_traveler), ftt(failed_transit_traveler), gut(give_up_traveler), TM, rc(risk city)
#tt(flow), ftt(stock), gut(stock), rc(stock)

# 3days
# create blank frames to store means of repeated simulations
means_integrals_3days_R <- data.frame(matrix(NA, nrow = nrow(anc_3days_for_R), ncol = 70))
means_integrals_3days_M <- data.frame(matrix(NA, nrow = nrow(anc_3days_for_M), ncol = 70))
means_integrals_3days_tt <- data.frame(matrix(NA, nrow = nrow(anc_3days), ncol = 70))
means_3days_ftt <- data.frame(matrix(NA, nrow = nrow(anc_3days), ncol = 35))
means_3days_gut <- data.frame(matrix(NA, nrow = nrow(anc_3days), ncol = 35))
means_integrals_3days_TM <- data.frame(matrix(NA, nrow = nrow(anc_3days), ncol = 70))
means_3days_rc <- data.frame(matrix(NA, nrow = nrow(anc_3days), ncol = 35))
# computing means of repeated groups and rename the columns
anc_3days_for_tt <- anc_3days %>%
  select(seq(6, 3147, by = 9))
anc_3days_for_ftt <- anc_3days %>%
  select(seq(7, 3148, by = 9))
anc_3days_for_gut <- anc_3days %>%
  select(seq(8, 3149, by = 9))
anc_3days_for_TM <- anc_3days %>%
  select(seq(3, 3144, by = 9))
anc_3days_for_rc <- anc_3days %>%
...
for (i in 1:35) {
  ...
  testgroup_ftt <- anc_3days_for_ftt %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
...
rm(anc_3days_for_R, anc_3days_for_M, anc_3days_for_tt, anc_3days_for_ftt, anc_3days_for_gut, anc_3days_for_TM, anc_3days_for_rc)

# 7days
# create blank frames to store means of repeated simulations
means_integrals_7days_R <- data.frame(matrix(NA, nrow = nrow(anc_7days_for_R), ncol = 70))
means_integrals_7days_M <- data.frame(matrix(NA, nrow = nrow(anc_7days_for_M), ncol = 70))
means_integrals_7days_tt <- data.frame(matrix(NA, nrow = nrow(anc_7days), ncol = 70))
means_7days_ftt <- data.frame(matrix(NA, nrow = nrow(anc_7days), ncol = 35))
means_7days_gut <- data.frame(matrix(NA, nrow = nrow(anc_7days), ncol = 35))
means_integrals_7days_TM <- data.frame(matrix(NA, nrow = nrow(anc_7days), ncol = 70))
means_7days_rc <- data.frame(matrix(NA, nrow = nrow(anc_7days), ncol = 35))
# computing means of repeated groups and rename the columns
anc_7days_for_tt <- anc_7days %>%
  select(seq(6, 3147, by = 9))
anc_7days_for_ftt <- anc_7days %>%
  select(seq(7, 3148, by = 9))
anc_7days_for_gut <- anc_7days %>%
  select(seq(8, 3149, by = 9))
anc_7days_for_TM <- anc_7days %>%
  select(seq(3, 3144, by = 9))
anc_7days_for_rc <- anc_7days %>%
  select(seq(9, 3150, by = 9))
...
for (i in 1:35) {
  ...
  # rc: risk_city
  testgroup_rc <- anc_7days_for_rc %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  means_7days_rc[,i] <- rowMeans(testgroup_rc)
  colnames(means_7days_rc)[i] <- paste("rcintegral", combinations[3,i], sep = "_")
}
rm(testgroup_R, testgroup_M, testgroup_tt, testgroup_ftt, testgroup_gut, testgroup_TM, testgroup_rc)
rm(anc_7days_for_R, anc_7days_for_M, anc_7days_for_tt, anc_7days_for_ftt, anc_7days_for_gut, anc_7days_for_TM, anc_7days_for_rc)

# 14days
# create blank frames to store means of repeated simulations
means_integrals_14days_R <- data.frame(matrix(NA, nrow = nrow(anc_14days_for_R), ncol = 70))
means_integrals_14days_M <- data.frame(matrix(NA, nrow = nrow(anc_14days_for_M), ncol = 70))
means_integrals_14days_tt <- data.frame(matrix(NA, nrow = nrow(anc_14days), ncol = 70))
means_14days_ftt <- data.frame(matrix(NA, nrow = nrow(anc_14days), ncol = 35))
means_14days_gut <- data.frame(matrix(NA, nrow = nrow(anc_14days), ncol = 35))
means_integrals_14days_TM <- data.frame(matrix(NA, nrow = nrow(anc_14days), ncol = 70))
means_14days_rc <- data.frame(matrix(NA, nrow = nrow(anc_14days), ncol = 35))
# computing means of repeated groups and rename the columns
anc_14days_for_tt <- anc_14days %>%
  select(seq(6, 3147, by = 9))
anc_14days_for_ftt <- anc_14days %>%
  select(seq(7, 3148, by = 9))
anc_14days_for_gut <- anc_14days %>%
  select(seq(8, 3149, by = 9))
anc_14days_for_TM <- anc_14days %>%
  select(seq(3, 3144, by = 9))
anc_14days_for_rc <- anc_14days %>%
  select(seq(9, 3150, by = 9))
...
for (i in 1:35) {
  ...
  # tt: transit_traveler
  testgroup_tt <- anc_14days_for_tt %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  ...
  # rc: risk_city
  testgroup_rc <- anc_14days_for_rc %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  means_14days_rc[,i] <- rowMeans(testgroup_rc)
  colnames(means_14days_rc)[i] <- paste("rcintegral", combinations[3,i], sep = "_")
}
rm(testgroup_R, testgroup_M, testgroup_tt, testgroup_ftt, testgroup_gut, testgroup_TM, testgroup_rc)
rm(anc_14days_for_R, anc_14days_for_M, anc_14days_for_tt, anc_14days_for_ftt, anc_14days_for_gut, anc_14days_for_TM, anc_14days_for_rc)

# 21days
# create blank frames to store means of repeated simulations
means_integrals_21days_R <- data.frame(matrix(NA, nrow = nrow(anc_21days_for_R), ncol = 70))
means_integrals_21days_M <- data.frame(matrix(NA, nrow = nrow(anc_21days_for_M), ncol = 70))
means_integrals_21days_tt <- data.frame(matrix(NA, nrow = nrow(anc_21days), ncol = 70))
means_21days_ftt <- data.frame(matrix(NA, nrow = nrow(anc_21days), ncol = 35))
means_21days_gut <- data.frame(matrix(NA, nrow = nrow(anc_21days), ncol = 35))
means_integrals_21days_TM <- data.frame(matrix(NA, nrow = nrow(anc_21days), ncol = 70))
means_21days_rc <- data.frame(matrix(NA, nrow = nrow(anc_21days), ncol = 35))
# computing means of repeated groups and rename the columns
anc_21days_for_tt <- anc_21days %>%
  select(seq(6, 3147, by = 9))
anc_21days_for_ftt <- anc_21days %>%
  select(seq(7, 3148, by = 9))
anc_21days_for_gut <- anc_21days %>%
  select(seq(8, 3149, by = 9))
anc_21days_for_TM <- anc_21days %>%
  select(seq(3, 3144, by = 9))
anc_21days_for_rc <- anc_21days %>%
  select(seq(9, 3150, by = 9))
...
for (i in 1:35) {
  ...
  # tt: transit_traveler
  testgroup_tt <- anc_21days_for_tt %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  ...
  # rc: risk_city
  testgroup_rc <- anc_21days_for_rc %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  means_21days_rc[,i] <- rowMeans(testgroup_rc)
  colnames(means_21days_rc)[i] <- paste("rcintegral", combinations[3,i], sep = "_")
}
rm(testgroup_R, testgroup_M, testgroup_tt, testgroup_ftt, testgroup_gut, testgroup_TM, testgroup_rc)
rm(anc_21days_for_R, anc_21days_for_M, anc_21days_for_tt, anc_21days_for_ftt, anc_21days_for_gut, anc_21days_for_TM, anc_21days_for_rc)
##------------------------
...
plot_means_3days_all <- plot_for_allgroup_means %>%
  ...
  labs(
    x = "tick",
    title = "3 days + arrival + no cooperation"
  )
...
plot_means_7days_all <- plot_for_allgroup_means %>%
  ...
  labs(
    x = "tick",
    title = "7 days + arrival + no cooperation"
  )
...
plot_means_14days_all <- plot_for_allgroup_means %>%
  ...
  labs(
    x = "tick",
    title = "14 days + arrival + no cooperation"
  )
...
plot_means_21days_all <- plot_for_allgroup_means %>%
  ...
  labs(
    x = "tick",
    title = "21 days + arrival + no cooperation"
  )
...
# 利用ggarrange将图像排列在一起
plot_RM_anc <- ggarrange(plot_means_3days_all, plot_means_7days_all, plot_means_14days_all, plot_means_21days_all,
...
write.xlsx(l, "arrival and no cooperation-data.xlsx")
##------------------------
