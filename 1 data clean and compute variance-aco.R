#--------------------
#Objective: 毕业论文数据处理与分析-arrival and united policy
...
raw_aco <- read.csv("leakage-arrival and cooperation one experiment-arrival and cooperation one-spreadsheet.csv")
##------------------------

##--Formatting--
# time_option: 3, 7, 14, 21 4*350=1400
# aco
combinations <- raw_aco[c(2,3),]
...
aco <- raw_aco[c(11:nrow(raw_aco)),]
rownames(aco) <- 1:nrow(aco)
aco <- select(aco, -1)
aco[1,] <- gsub(pattern = "compute-", replacement = "", aco[1,])
aco_3days <- select(aco, 1:3150)
aco_7days <- select(aco, 3151:6300)
aco_14days <- select(aco, 6301:9450)
aco_21days <- select(aco, 9451:12600)
for (i in 1:ncol(combinations)) {
  for (j in 1:10) {
    for (w in 1:9) {
      colnames(aco_3days)[(i-1)*90+(j-1)*9+w] <- paste(aco_3days[1,(i-1)*90+(j-1)*9+w], j, combinations[3,i], sep = "_")
      colnames(aco_7days)[(i-1)*90+(j-1)*9+w] <- paste(aco_7days[1,(i-1)*90+(j-1)*9+w], j, combinations[3,i], sep = "_")
      colnames(aco_14days)[(i-1)*90+(j-1)*9+w] <- paste(aco_14days[1,(i-1)*90+(j-1)*9+w], j, combinations[3,i], sep = "_")
      colnames(aco_21days)[(i-1)*90+(j-1)*9+w] <- paste(aco_21days[1,(i-1)*90+(j-1)*9+w], j, combinations[3,i], sep = "_")
    }
  }
}
aco_3days <- aco_3days[-c(1,2),]
aco_7days <- aco_7days[-c(1,2),]
aco_14days <- aco_14days[-c(1,2),]
aco_21days <- aco_21days[-c(1,2),]
rownames(aco_3days) <- 1:nrow(aco_3days)
rownames(aco_7days) <- 1:nrow(aco_7days)
rownames(aco_14days) <- 1:nrow(aco_14days)
rownames(aco_21days) <- 1:nrow(aco_21days)
for (i in 1:ncol(aco_3days)) {
  aco_3days[,i] <- as.numeric(aco_3days[,i])
  aco_7days[,i] <- as.numeric(aco_7days[,i])
  aco_14days[,i] <- as.numeric(aco_14days[,i])
  aco_21days[,i] <- as.numeric(aco_21days[,i])
}
##------------------------
...
# aco: R, M
# 3days
aco_3days_for_R <- aco_3days %>%
  select(seq(1, 3142, by = 9))
aco_3days_for_M <- aco_3days %>%
  select(seq(2, 3143, by = 9))

# create blank frames to store variances of variables
variance_3days_R <- data.frame(matrix(NA, nrow = nrow(aco_3days_for_R), ncol = 35))
variance_3days_M <- data.frame(matrix(NA, nrow = nrow(aco_3days_for_M), ncol = 35))
# computing variances of repeated groups and rename the columns
for (i in 1:35) {
  # R: Risk
  testgroup_R <- aco_3days_for_R %>%
...
# 7days
aco_7days_for_R <- aco_7days %>%
  select(seq(1, 3142, by = 9))
aco_7days_for_M <- aco_7days %>%
  select(seq(2, 3143, by = 9))

# create blank frames to store variances of variables
variance_7days_R <- data.frame(matrix(NA, nrow = nrow(aco_7days_for_R), ncol = 35))
variance_7days_M <- data.frame(matrix(NA, nrow = nrow(aco_7days_for_M), ncol = 35))
# computing variances of repeated groups and rename the columns
for (i in 1:35) {
  # R: Risk
  testgroup_R <- aco_7days_for_R %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  testgroup_R <- as.matrix(testgroup_R)
  variance_7days_R[,i] <- rowSds(testgroup_R)
  colnames(variance_7days_R)[i] <- paste("Rsd", combinations[3,i], sep = "_")
  # M: Mobility
  testgroup_M <- aco_7days_for_M %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  testgroup_M <- as.matrix(testgroup_M)
...
# 14days
aco_14days_for_R <- aco_14days %>%
  select(seq(1, 3142, by = 9))
aco_14days_for_M <- aco_14days %>%
  select(seq(2, 3143, by = 9))

# create blank frames to store variances of variables
variance_14days_R <- data.frame(matrix(NA, nrow = nrow(aco_14days_for_R), ncol = 35))
variance_14days_M <- data.frame(matrix(NA, nrow = nrow(aco_14days_for_M), ncol = 35))
# computing variances of repeated groups and rename the columns
for (i in 1:35) {
  # R: Risk
  testgroup_R <- aco_14days_for_R %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  testgroup_R <- as.matrix(testgroup_R)
  variance_14days_R[,i] <- rowSds(testgroup_R)
  colnames(variance_14days_R)[i] <- paste("Rsd", combinations[3,i], sep = "_")
  # M: Mobility
  testgroup_M <- aco_14days_for_M %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  testgroup_M <- as.matrix(testgroup_M)
...
# 21days
aco_21days_for_R <- aco_21days %>%
  select(seq(1, 3142, by = 9))
aco_21days_for_M <- aco_21days %>%
  select(seq(2, 3143, by = 9))

# create blank frames to store variances of variables
variance_21days_R <- data.frame(matrix(NA, nrow = nrow(aco_21days_for_R), ncol = 35))
variance_21days_M <- data.frame(matrix(NA, nrow = nrow(aco_21days_for_M), ncol = 35))
# computing variances of repeated groups and rename the columns
for (i in 1:35) {
  # R: Risk
  testgroup_R <- aco_21days_for_R %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  testgroup_R <- as.matrix(testgroup_R)
  variance_21days_R[,i] <- rowSds(testgroup_R)
  colnames(variance_21days_R)[i] <- paste("Rsd", combinations[3,i], sep = "_")
  # M: Mobility
  testgroup_M <- aco_21days_for_M %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  testgroup_M <- as.matrix(testgroup_M)
...
# 利用ggarrange将图像排列在一起
plot_variance_aco <- ggarrange(plot_variance_3days_R, plot_variance_7days_R, plot_variance_14days_R, plot_variance_21days_R,
                               plot_variance_3days_M, plot_variance_7days_M, plot_variance_14days_M, plot_variance_21days_M,
                               ncol = 4,nrow =2)
plot_variance_aco # 手动export大小合适的图
...
##--Discrete integral--
# aco: R, M, tt(transit_traveler), ftt(failed_transit_traveler), gut(give_up_traveler), TM, rc(risk city)
#tt(flow), ftt(stock), gut(stock), rc(stock)

# 3days
# create blank frames to store means of repeated simulations
means_integrals_3days_R <- data.frame(matrix(NA, nrow = nrow(aco_3days_for_R), ncol = 70))
means_integrals_3days_M <- data.frame(matrix(NA, nrow = nrow(aco_3days_for_M), ncol = 70))
means_integrals_3days_tt <- data.frame(matrix(NA, nrow = nrow(aco_3days), ncol = 70))
means_3days_ftt <- data.frame(matrix(NA, nrow = nrow(aco_3days), ncol = 35))
means_3days_gut <- data.frame(matrix(NA, nrow = nrow(aco_3days), ncol = 35))
means_integrals_3days_TM <- data.frame(matrix(NA, nrow = nrow(aco_3days), ncol = 70))
means_3days_rc <- data.frame(matrix(NA, nrow = nrow(aco_3days), ncol = 35))
# computing means of repeated groups and rename the columns
aco_3days_for_tt <- aco_3days %>%
  select(seq(6, 3147, by = 9))
aco_3days_for_ftt <- aco_3days %>%
  select(seq(7, 3148, by = 9))
aco_3days_for_gut <- aco_3days %>%
  select(seq(8, 3149, by = 9))
aco_3days_for_TM <- aco_3days %>%
  select(seq(3, 3144, by = 9))
aco_3days_for_rc <- aco_3days %>%
...
for (i in 1:35) {
  ...
  testgroup_ftt <- aco_3days_for_ftt %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
...
rm(aco_3days_for_R, aco_3days_for_M, aco_3days_for_tt, aco_3days_for_ftt, aco_3days_for_gut, aco_3days_for_TM, aco_3days_for_rc)

# 7days
# create blank frames to store means of repeated simulations
means_integrals_7days_R <- data.frame(matrix(NA, nrow = nrow(aco_7days_for_R), ncol = 70))
means_integrals_7days_M <- data.frame(matrix(NA, nrow = nrow(aco_7days_for_M), ncol = 70))
means_integrals_7days_tt <- data.frame(matrix(NA, nrow = nrow(aco_7days), ncol = 70))
means_7days_ftt <- data.frame(matrix(NA, nrow = nrow(aco_7days), ncol = 35))
means_7days_gut <- data.frame(matrix(NA, nrow = nrow(aco_7days), ncol = 35))
means_integrals_7days_TM <- data.frame(matrix(NA, nrow = nrow(aco_7days), ncol = 70))
means_7days_rc <- data.frame(matrix(NA, nrow = nrow(aco_7days), ncol = 35))
# computing means of repeated groups and rename the columns
aco_7days_for_tt <- aco_7days %>%
  select(seq(6, 3147, by = 9))
aco_7days_for_ftt <- aco_7days %>%
  select(seq(7, 3148, by = 9))
aco_7days_for_gut <- aco_7days %>%
  select(seq(8, 3149, by = 9))
aco_7days_for_TM <- aco_7days %>%
  select(seq(3, 3144, by = 9))
aco_7days_for_rc <- aco_7days %>%
  select(seq(9, 3150, by = 9))
...
for (i in 1:35) {
  ...
  # rc: risk_city
  testgroup_rc <- aco_7days_for_rc %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  means_7days_rc[,i] <- rowMeans(testgroup_rc)
  colnames(means_7days_rc)[i] <- paste("rcintegral", combinations[3,i], sep = "_")
}
rm(testgroup_R, testgroup_M, testgroup_tt, testgroup_ftt, testgroup_gut, testgroup_TM, testgroup_rc)
rm(aco_7days_for_R, aco_7days_for_M, aco_7days_for_tt, aco_7days_for_ftt, aco_7days_for_gut, aco_7days_for_TM, aco_7days_for_rc)

# 14days
# create blank frames to store means of repeated simulations
means_integrals_14days_R <- data.frame(matrix(NA, nrow = nrow(aco_14days_for_R), ncol = 70))
means_integrals_14days_M <- data.frame(matrix(NA, nrow = nrow(aco_14days_for_M), ncol = 70))
means_integrals_14days_tt <- data.frame(matrix(NA, nrow = nrow(aco_14days), ncol = 70))
means_14days_ftt <- data.frame(matrix(NA, nrow = nrow(aco_14days), ncol = 35))
means_14days_gut <- data.frame(matrix(NA, nrow = nrow(aco_14days), ncol = 35))
means_integrals_14days_TM <- data.frame(matrix(NA, nrow = nrow(aco_14days), ncol = 70))
means_14days_rc <- data.frame(matrix(NA, nrow = nrow(aco_14days), ncol = 35))
# computing means of repeated groups and rename the columns
aco_14days_for_tt <- aco_14days %>%
  select(seq(6, 3147, by = 9))
aco_14days_for_ftt <- aco_14days %>%
  select(seq(7, 3148, by = 9))
aco_14days_for_gut <- aco_14days %>%
  select(seq(8, 3149, by = 9))
aco_14days_for_TM <- aco_14days %>%
  select(seq(3, 3144, by = 9))
aco_14days_for_rc <- aco_14days %>%
  select(seq(9, 3150, by = 9))
...
for (i in 1:35) {
  ...
  # tt: transit_traveler
  testgroup_tt <- aco_14days_for_tt %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  ...
  # rc: risk_city
  testgroup_rc <- aco_14days_for_rc %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  means_14days_rc[,i] <- rowMeans(testgroup_rc)
  colnames(means_14days_rc)[i] <- paste("rcintegral", combinations[3,i], sep = "_")
}
rm(testgroup_R, testgroup_M, testgroup_tt, testgroup_ftt, testgroup_gut, testgroup_TM, testgroup_rc)
rm(aco_14days_for_R, aco_14days_for_M, aco_14days_for_tt, aco_14days_for_ftt, aco_14days_for_gut, aco_14days_for_TM, aco_14days_for_rc)

# 21days
# create blank frames to store means of repeated simulations
means_integrals_21days_R <- data.frame(matrix(NA, nrow = nrow(aco_21days_for_R), ncol = 70))
means_integrals_21days_M <- data.frame(matrix(NA, nrow = nrow(aco_21days_for_M), ncol = 70))
means_integrals_21days_tt <- data.frame(matrix(NA, nrow = nrow(aco_21days), ncol = 70))
means_21days_ftt <- data.frame(matrix(NA, nrow = nrow(aco_21days), ncol = 35))
means_21days_gut <- data.frame(matrix(NA, nrow = nrow(aco_21days), ncol = 35))
means_integrals_21days_TM <- data.frame(matrix(NA, nrow = nrow(aco_21days), ncol = 70))
means_21days_rc <- data.frame(matrix(NA, nrow = nrow(aco_21days), ncol = 35))
# computing means of repeated groups and rename the columns
aco_21days_for_tt <- aco_21days %>%
  select(seq(6, 3147, by = 9))
aco_21days_for_ftt <- aco_21days %>%
  select(seq(7, 3148, by = 9))
aco_21days_for_gut <- aco_21days %>%
  select(seq(8, 3149, by = 9))
aco_21days_for_TM <- aco_21days %>%
  select(seq(3, 3144, by = 9))
aco_21days_for_rc <- aco_21days %>%
  select(seq(9, 3150, by = 9))
...
for (i in 1:35) {
  ...
  # tt: transit_traveler
  testgroup_tt <- aco_21days_for_tt %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  ...
  # rc: risk_city
  testgroup_rc <- aco_21days_for_rc %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  means_21days_rc[,i] <- rowMeans(testgroup_rc)
  colnames(means_21days_rc)[i] <- paste("rcintegral", combinations[3,i], sep = "_")
}
rm(testgroup_R, testgroup_M, testgroup_tt, testgroup_ftt, testgroup_gut, testgroup_TM, testgroup_rc)
rm(aco_21days_for_R, aco_21days_for_M, aco_21days_for_tt, aco_21days_for_ftt, aco_21days_for_gut, aco_21days_for_TM, aco_21days_for_rc)
##------------------------
...
plot_means_3days_all <- plot_for_allgroup_means %>%
  ...
  labs(
    x = "tick",
    title = "3 days + arrival + united restriction"
  )
...
plot_means_7days_all <- plot_for_allgroup_means %>%
  ...
  labs(
    x = "tick",
    title = "7 days + arrival + united restriction"
  )
...
plot_means_14days_all <- plot_for_allgroup_means %>%
  ...
  labs(
    x = "tick",
    title = "14 days + arrival + united restriction"
  )
...
plot_means_21days_all <- plot_for_allgroup_means %>%
  ...
  labs(
    x = "tick",
    title = "21 days + arrival + united restriction"
  )
...
# 利用ggarrange将图像排列在一起
plot_RM_aco <- ggarrange(plot_means_3days_all, plot_means_7days_all, plot_means_14days_all, plot_means_21days_all,
...
write.xlsx(l, "arrival and united policy-data.xlsx")
##------------------------
