...
to setup
  ...
  ifelse time_option = 3 [
    set threshold 0.5896
  ][
    ifelse time_option = 7 [
      set threshold 0.5416
    ][
      ifelse time_option = 14 [
        set threshold 0.5581
      ][
        set threshold 0.5274
      ]
    ]
  ]
...
to go
  ...
  ask new_turtles[
    ifelse [risk] of departure = 0 [ ;;非涉疫城市出发
      ...
      ifelse destination = patch 0 0 [ ;;如果目的地是核心城市，则选择是否避难，去哪避难
        transit-or-not
        find-sanctuary
      ][ ;;如果目的地是其他城市，则直接出行
        move-to destination
...
to transit-or-not
  ;;是否避难
  ifelse transit_preference > threshold [ ;;避难偏好大于方案阈值就会选择去避难，否则放弃而离开系统
    set transition 1
  ][
...
@#$#@#$#@
<experiments>
  <experiment name="experiment-departure and no cooperation" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="70"/>
  ...
  <experiment name="experiment-14days-80,800" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="70"/>
    <metric>compute-R</metric>
