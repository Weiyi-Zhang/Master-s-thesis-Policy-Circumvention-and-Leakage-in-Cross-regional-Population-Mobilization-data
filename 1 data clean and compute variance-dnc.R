#--------------------
#Objective: 毕业论文数据处理与分析-departure and no cooperation
...
raw_dnc <- read.csv("leakage-departure and no cooperation experiment-departure and no cooperation-spreadsheet.csv")
##------------------------

##--Formatting--
# time_option: 3, 7, 14, 21 4*350=1400
# dnc
combinations <- raw_dnc[c(2,3),]
...
dnc <- raw_dnc[c(11:nrow(raw_dnc)),]
rownames(dnc) <- 1:nrow(dnc)
dnc <- select(dnc, -1)
dnc[1,] <- gsub(pattern = "compute-", replacement = "", dnc[1,])
dnc_3days <- select(dnc, 1:3150)
dnc_7days <- select(dnc, 3151:6300)
dnc_14days <- select(dnc, 6301:9450)
dnc_21days <- select(dnc, 9451:12600)
for (i in 1:ncol(combinations)) {
  for (j in 1:10) {
    for (w in 1:9) {
      colnames(dnc_3days)[(i-1)*90+(j-1)*9+w] <- paste(dnc_3days[1,(i-1)*90+(j-1)*9+w], j, combinations[3,i], sep = "_")
      colnames(dnc_7days)[(i-1)*90+(j-1)*9+w] <- paste(dnc_7days[1,(i-1)*90+(j-1)*9+w], j, combinations[3,i], sep = "_")
      colnames(dnc_14days)[(i-1)*90+(j-1)*9+w] <- paste(dnc_14days[1,(i-1)*90+(j-1)*9+w], j, combinations[3,i], sep = "_")
      colnames(dnc_21days)[(i-1)*90+(j-1)*9+w] <- paste(dnc_21days[1,(i-1)*90+(j-1)*9+w], j, combinations[3,i], sep = "_")
    }
  }
}
dnc_3days <- dnc_3days[-c(1,2),]
dnc_7days <- dnc_7days[-c(1,2),]
dnc_14days <- dnc_14days[-c(1,2),]
dnc_21days <- dnc_21days[-c(1,2),]
rownames(dnc_3days) <- 1:nrow(dnc_3days)
rownames(dnc_7days) <- 1:nrow(dnc_7days)
rownames(dnc_14days) <- 1:nrow(dnc_14days)
rownames(dnc_21days) <- 1:nrow(dnc_21days)
for (i in 1:ncol(dnc_3days)) {
  dnc_3days[,i] <- as.numeric(dnc_3days[,i])
  dnc_7days[,i] <- as.numeric(dnc_7days[,i])
  dnc_14days[,i] <- as.numeric(dnc_14days[,i])
  dnc_21days[,i] <- as.numeric(dnc_21days[,i])
}
##------------------------
...
# dnc: R, M
# 3days
dnc_3days_for_R <- dnc_3days %>%
  select(seq(1, 3142, by = 9))
dnc_3days_for_M <- dnc_3days %>%
  select(seq(2, 3143, by = 9))

# create blank frames to store variances of variables
variance_3days_R <- data.frame(matrix(NA, nrow = nrow(dnc_3days_for_R), ncol = 35))
variance_3days_M <- data.frame(matrix(NA, nrow = nrow(dnc_3days_for_M), ncol = 35))
# computing variances of repeated groups and rename the columns
for (i in 1:35) {
  # R: Risk
  testgroup_R <- dnc_3days_for_R %>%
...
# 7days
dnc_7days_for_R <- dnc_7days %>%
  select(seq(1, 3142, by = 9))
dnc_7days_for_M <- dnc_7days %>%
  select(seq(2, 3143, by = 9))

# create blank frames to store variances of variables
variance_7days_R <- data.frame(matrix(NA, nrow = nrow(dnc_7days_for_R), ncol = 35))
variance_7days_M <- data.frame(matrix(NA, nrow = nrow(dnc_7days_for_M), ncol = 35))
# computing variances of repeated groups and rename the columns
for (i in 1:35) {
  # R: Risk
  testgroup_R <- dnc_7days_for_R %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  testgroup_R <- as.matrix(testgroup_R)
  variance_7days_R[,i] <- rowSds(testgroup_R)
  colnames(variance_7days_R)[i] <- paste("Rsd", combinations[3,i], sep = "_")
  # M: Mobility
  testgroup_M <- dnc_7days_for_M %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  testgroup_M <- as.matrix(testgroup_M)
...
# 14days
dnc_14days_for_R <- dnc_14days %>%
  select(seq(1, 3142, by = 9))
dnc_14days_for_M <- dnc_14days %>%
  select(seq(2, 3143, by = 9))

# create blank frames to store variances of variables
variance_14days_R <- data.frame(matrix(NA, nrow = nrow(dnc_14days_for_R), ncol = 35))
variance_14days_M <- data.frame(matrix(NA, nrow = nrow(dnc_14days_for_M), ncol = 35))
# computing variances of repeated groups and rename the columns
for (i in 1:35) {
  # R: Risk
  testgroup_R <- dnc_14days_for_R %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  testgroup_R <- as.matrix(testgroup_R)
  variance_14days_R[,i] <- rowSds(testgroup_R)
  colnames(variance_14days_R)[i] <- paste("Rsd", combinations[3,i], sep = "_")
  # M: Mobility
  testgroup_M <- dnc_14days_for_M %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  testgroup_M <- as.matrix(testgroup_M)
...
# 21days
dnc_21days_for_R <- dnc_21days %>%
  select(seq(1, 3142, by = 9))
dnc_21days_for_M <- dnc_21days %>%
  select(seq(2, 3143, by = 9))

# create blank frames to store variances of variables
variance_21days_R <- data.frame(matrix(NA, nrow = nrow(dnc_21days_for_R), ncol = 35))
variance_21days_M <- data.frame(matrix(NA, nrow = nrow(dnc_21days_for_M), ncol = 35))
# computing variances of repeated groups and rename the columns
for (i in 1:35) {
  # R: Risk
  testgroup_R <- dnc_21days_for_R %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  testgroup_R <- as.matrix(testgroup_R)
  variance_21days_R[,i] <- rowSds(testgroup_R)
  colnames(variance_21days_R)[i] <- paste("Rsd", combinations[3,i], sep = "_")
  # M: Mobility
  testgroup_M <- dnc_21days_for_M %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  testgroup_M <- as.matrix(testgroup_M)
...
# 利用ggarrange将图像排列在一起
plot_variance_dnc <- ggarrange(plot_variance_3days_R, plot_variance_7days_R, plot_variance_14days_R, plot_variance_21days_R,
                               plot_variance_3days_M, plot_variance_7days_M, plot_variance_14days_M, plot_variance_21days_M,
                               ncol = 4,nrow =2)
plot_variance_dnc # 手动export大小合适的图
...
##--Discrete integral--
# dnc: R, M, tt(transit_traveler), ftt(failed_transit_traveler), gut(give_up_traveler), TM, rc(risk city)
#tt(flow), ftt(stock), gut(stock), rc(stock)

# 3days
# create blank frames to store means of repeated simulations
means_integrals_3days_R <- data.frame(matrix(NA, nrow = nrow(dnc_3days_for_R), ncol = 70))
means_integrals_3days_M <- data.frame(matrix(NA, nrow = nrow(dnc_3days_for_M), ncol = 70))
means_integrals_3days_tt <- data.frame(matrix(NA, nrow = nrow(dnc_3days), ncol = 70))
means_3days_ftt <- data.frame(matrix(NA, nrow = nrow(dnc_3days), ncol = 35))
means_3days_gut <- data.frame(matrix(NA, nrow = nrow(dnc_3days), ncol = 35))
means_integrals_3days_TM <- data.frame(matrix(NA, nrow = nrow(dnc_3days), ncol = 70))
means_3days_rc <- data.frame(matrix(NA, nrow = nrow(dnc_3days), ncol = 35))
# computing means of repeated groups and rename the columns
dnc_3days_for_tt <- dnc_3days %>%
  select(seq(6, 3147, by = 9))
dnc_3days_for_ftt <- dnc_3days %>%
  select(seq(7, 3148, by = 9))
dnc_3days_for_gut <- dnc_3days %>%
  select(seq(8, 3149, by = 9))
dnc_3days_for_TM <- dnc_3days %>%
  select(seq(3, 3144, by = 9))
dnc_3days_for_rc <- dnc_3days %>%
...
for (i in 1:35) {
  ...
  testgroup_ftt <- dnc_3days_for_ftt %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
...
rm(dnc_3days_for_R, dnc_3days_for_M, dnc_3days_for_tt, dnc_3days_for_ftt, dnc_3days_for_gut, dnc_3days_for_TM, dnc_3days_for_rc)

# 7days
# create blank frames to store means of repeated simulations
means_integrals_7days_R <- data.frame(matrix(NA, nrow = nrow(dnc_7days_for_R), ncol = 70))
means_integrals_7days_M <- data.frame(matrix(NA, nrow = nrow(dnc_7days_for_M), ncol = 70))
means_integrals_7days_tt <- data.frame(matrix(NA, nrow = nrow(dnc_7days), ncol = 70))
means_7days_ftt <- data.frame(matrix(NA, nrow = nrow(dnc_7days), ncol = 35))
means_7days_gut <- data.frame(matrix(NA, nrow = nrow(dnc_7days), ncol = 35))
means_integrals_7days_TM <- data.frame(matrix(NA, nrow = nrow(dnc_7days), ncol = 70))
means_7days_rc <- data.frame(matrix(NA, nrow = nrow(dnc_7days), ncol = 35))
# computing means of repeated groups and rename the columns
dnc_7days_for_tt <- dnc_7days %>%
  select(seq(6, 3147, by = 9))
dnc_7days_for_ftt <- dnc_7days %>%
  select(seq(7, 3148, by = 9))
dnc_7days_for_gut <- dnc_7days %>%
  select(seq(8, 3149, by = 9))
dnc_7days_for_TM <- dnc_7days %>%
  select(seq(3, 3144, by = 9))
dnc_7days_for_rc <- dnc_7days %>%
  select(seq(9, 3150, by = 9))
...
for (i in 1:35) {
  ...
  # rc: risk_city
  testgroup_rc <- dnc_7days_for_rc %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  means_7days_rc[,i] <- rowMeans(testgroup_rc)
  colnames(means_7days_rc)[i] <- paste("rcintegral", combinations[3,i], sep = "_")
}
rm(testgroup_R, testgroup_M, testgroup_tt, testgroup_ftt, testgroup_gut, testgroup_TM, testgroup_rc)
rm(dnc_7days_for_R, dnc_7days_for_M, dnc_7days_for_tt, dnc_7days_for_ftt, dnc_7days_for_gut, dnc_7days_for_TM, dnc_7days_for_rc)

# 14days
# create blank frames to store means of repeated simulations
means_integrals_14days_R <- data.frame(matrix(NA, nrow = nrow(dnc_14days_for_R), ncol = 70))
means_integrals_14days_M <- data.frame(matrix(NA, nrow = nrow(dnc_14days_for_M), ncol = 70))
means_integrals_14days_tt <- data.frame(matrix(NA, nrow = nrow(dnc_14days), ncol = 70))
means_14days_ftt <- data.frame(matrix(NA, nrow = nrow(dnc_14days), ncol = 35))
means_14days_gut <- data.frame(matrix(NA, nrow = nrow(dnc_14days), ncol = 35))
means_integrals_14days_TM <- data.frame(matrix(NA, nrow = nrow(dnc_14days), ncol = 70))
means_14days_rc <- data.frame(matrix(NA, nrow = nrow(dnc_14days), ncol = 35))
# computing means of repeated groups and rename the columns
dnc_14days_for_tt <- dnc_14days %>%
  select(seq(6, 3147, by = 9))
dnc_14days_for_ftt <- dnc_14days %>%
  select(seq(7, 3148, by = 9))
dnc_14days_for_gut <- dnc_14days %>%
  select(seq(8, 3149, by = 9))
dnc_14days_for_TM <- dnc_14days %>%
  select(seq(3, 3144, by = 9))
dnc_14days_for_rc <- dnc_14days %>%
  select(seq(9, 3150, by = 9))
...
for (i in 1:35) {
  ...
  # tt: transit_traveler
  testgroup_tt <- dnc_14days_for_tt %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  ...
  # rc: risk_city
  testgroup_rc <- dnc_14days_for_rc %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  means_14days_rc[,i] <- rowMeans(testgroup_rc)
  colnames(means_14days_rc)[i] <- paste("rcintegral", combinations[3,i], sep = "_")
}
rm(testgroup_R, testgroup_M, testgroup_tt, testgroup_ftt, testgroup_gut, testgroup_TM, testgroup_rc)
rm(dnc_14days_for_R, dnc_14days_for_M, dnc_14days_for_tt, dnc_14days_for_ftt, dnc_14days_for_gut, dnc_14days_for_TM, dnc_14days_for_rc)

# 21days
# create blank frames to store means of repeated simulations
means_integrals_21days_R <- data.frame(matrix(NA, nrow = nrow(dnc_21days_for_R), ncol = 70))
means_integrals_21days_M <- data.frame(matrix(NA, nrow = nrow(dnc_21days_for_M), ncol = 70))
means_integrals_21days_tt <- data.frame(matrix(NA, nrow = nrow(dnc_21days), ncol = 70))
means_21days_ftt <- data.frame(matrix(NA, nrow = nrow(dnc_21days), ncol = 35))
means_21days_gut <- data.frame(matrix(NA, nrow = nrow(dnc_21days), ncol = 35))
means_integrals_21days_TM <- data.frame(matrix(NA, nrow = nrow(dnc_21days), ncol = 70))
means_21days_rc <- data.frame(matrix(NA, nrow = nrow(dnc_21days), ncol = 35))
# computing means of repeated groups and rename the columns
dnc_21days_for_tt <- dnc_21days %>%
  select(seq(6, 3147, by = 9))
dnc_21days_for_ftt <- dnc_21days %>%
  select(seq(7, 3148, by = 9))
dnc_21days_for_gut <- dnc_21days %>%
  select(seq(8, 3149, by = 9))
dnc_21days_for_TM <- dnc_21days %>%
  select(seq(3, 3144, by = 9))
dnc_21days_for_rc <- dnc_21days %>%
  select(seq(9, 3150, by = 9))
...
for (i in 1:35) {
  ...
  # tt: transit_traveler
  testgroup_tt <- dnc_21days_for_tt %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  ...
  # rc: risk_city
  testgroup_rc <- dnc_21days_for_rc %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  means_21days_rc[,i] <- rowMeans(testgroup_rc)
  colnames(means_21days_rc)[i] <- paste("rcintegral", combinations[3,i], sep = "_")
}
rm(testgroup_R, testgroup_M, testgroup_tt, testgroup_ftt, testgroup_gut, testgroup_TM, testgroup_rc)
rm(dnc_21days_for_R, dnc_21days_for_M, dnc_21days_for_tt, dnc_21days_for_ftt, dnc_21days_for_gut, dnc_21days_for_TM, dnc_21days_for_rc)
##------------------------
...
plot_means_3days_all <- plot_for_allgroup_means %>%
  ...
  geom_area(aes(y = Rmeans*100), fill = "red") +
  scale_y_continuous(
    ...
    sec.axis = sec_axis(trans = ~./100, name = "Rmeans")
  ) +
  theme_bw() +
  labs(
    x = "tick",
    title = "3 days + departure + no cooperation"
  )
...
plot_integral_3days_all <- plot_for_allgroup_integrals %>%
  ...
  geom_area(aes(y = Rintegral*100), fill = "red") +
  scale_y_continuous(
    # features of the first axis
    name = "Mintegral",
    limits = c(0, 27500),
    ...
    # add a second axis and specify its features
    sec.axis = sec_axis(trans = ~./100, name = "Rintegral")
  ) +
  theme_bw() +
  labs(
    x = "tick",
...
plot_means_7days_all <- plot_for_allgroup_means %>%
  ...
  labs(
    x = "tick",
    title = "7 days + departure + no cooperation"
  )
...
plot_means_14days_all <- plot_for_allgroup_means %>%
  ...
  labs(
    x = "tick",
    title = "14 days + departure + no cooperation"
  )
...
plot_means_21days_all <- plot_for_allgroup_means %>%
  ...
  labs(
    x = "tick",
    title = "21 days + departure + no cooperation"
  )
...
# 利用ggarrange将图像排列在一起
plot_RM_dnc <- ggarrange(plot_means_3days_all, plot_means_7days_all, plot_means_14days_all, plot_means_21days_all,
                         plot_integral_3days_all, plot_integral_7days_all, plot_integral_14days_all, plot_integral_21days_all,
                         ncol = 4,nrow =2)

rm(plot_for_allgroup_means, plot_for_allgroup_integrals)
...
write.xlsx(l, "departure and no cooperation-data.xlsx")
##------------------------
