...
to setup
  ...
  ;;设置方案阈值(该方案下没有避难的可能性，但决定是否直接前往)
  ifelse time_option = 3 [
    set threshold2 0.5366
  ][
    ifelse time_option = 7 [
      set threshold2 0.5292
    ][
      ifelse time_option = 14 [
        set threshold2 0.4669
      ][
        set threshold2 0.4845
      ]
    ]
  ]
...
@#$#@#$#@
<experiments>
  <experiment name="experiment-arrival and united policy" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="70"/>
