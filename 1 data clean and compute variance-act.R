#--------------------
#Objective: 毕业论文数据处理与分析-arrival and data sharing
...
raw_act <- read.csv("leakage-arrival and cooperation two experiment-arrival and cooperation two-spreadsheet.csv")
##------------------------

##--Formatting--
# time_option: 3, 7, 14, 21 4*350=1400
# act
combinations <- raw_act[c(2,3),]
...
act <- raw_act[c(11:nrow(raw_act)),]
rownames(act) <- 1:nrow(act)
act <- select(act, -1)
act[1,] <- gsub(pattern = "compute-", replacement = "", act[1,])
act_3days <- select(act, 1:3150)
act_7days <- select(act, 3151:6300)
act_14days <- select(act, 6301:9450)
act_21days <- select(act, 9451:12600)
for (i in 1:ncol(combinations)) {
  for (j in 1:10) {
    for (w in 1:9) {
      colnames(act_3days)[(i-1)*90+(j-1)*9+w] <- paste(act_3days[1,(i-1)*90+(j-1)*9+w], j, combinations[3,i], sep = "_")
      colnames(act_7days)[(i-1)*90+(j-1)*9+w] <- paste(act_7days[1,(i-1)*90+(j-1)*9+w], j, combinations[3,i], sep = "_")
      colnames(act_14days)[(i-1)*90+(j-1)*9+w] <- paste(act_14days[1,(i-1)*90+(j-1)*9+w], j, combinations[3,i], sep = "_")
      colnames(act_21days)[(i-1)*90+(j-1)*9+w] <- paste(act_21days[1,(i-1)*90+(j-1)*9+w], j, combinations[3,i], sep = "_")
    }
  }
}
act_3days <- act_3days[-c(1,2),]
act_7days <- act_7days[-c(1,2),]
act_14days <- act_14days[-c(1,2),]
act_21days <- act_21days[-c(1,2),]
rownames(act_3days) <- 1:nrow(act_3days)
rownames(act_7days) <- 1:nrow(act_7days)
rownames(act_14days) <- 1:nrow(act_14days)
rownames(act_21days) <- 1:nrow(act_21days)
for (i in 1:ncol(act_3days)) {
  act_3days[,i] <- as.numeric(act_3days[,i])
  act_7days[,i] <- as.numeric(act_7days[,i])
  act_14days[,i] <- as.numeric(act_14days[,i])
  act_21days[,i] <- as.numeric(act_21days[,i])
}
##------------------------
...
# act: R, M
# 3days
act_3days_for_R <- act_3days %>%
  select(seq(1, 3142, by = 9))
act_3days_for_M <- act_3days %>%
  select(seq(2, 3143, by = 9))

# create blank frames to store variances of variables
variance_3days_R <- data.frame(matrix(NA, nrow = nrow(act_3days_for_R), ncol = 35))
variance_3days_M <- data.frame(matrix(NA, nrow = nrow(act_3days_for_M), ncol = 35))
# computing variances of repeated groups and rename the columns
for (i in 1:35) {
  # R: Risk
  testgroup_R <- act_3days_for_R %>%
...
# 7days
act_7days_for_R <- act_7days %>%
  select(seq(1, 3142, by = 9))
act_7days_for_M <- act_7days %>%
  select(seq(2, 3143, by = 9))

# create blank frames to store variances of variables
variance_7days_R <- data.frame(matrix(NA, nrow = nrow(act_7days_for_R), ncol = 35))
variance_7days_M <- data.frame(matrix(NA, nrow = nrow(act_7days_for_M), ncol = 35))
# computing variances of repeated groups and rename the columns
for (i in 1:35) {
  # R: Risk
  testgroup_R <- act_7days_for_R %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  testgroup_R <- as.matrix(testgroup_R)
  variance_7days_R[,i] <- rowSds(testgroup_R)
  colnames(variance_7days_R)[i] <- paste("Rsd", combinations[3,i], sep = "_")
  # M: Mobility
  testgroup_M <- act_7days_for_M %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  testgroup_M <- as.matrix(testgroup_M)
...
# 14days
act_14days_for_R <- act_14days %>%
  select(seq(1, 3142, by = 9))
act_14days_for_M <- act_14days %>%
  select(seq(2, 3143, by = 9))

# create blank frames to store variances of variables
variance_14days_R <- data.frame(matrix(NA, nrow = nrow(act_14days_for_R), ncol = 35))
variance_14days_M <- data.frame(matrix(NA, nrow = nrow(act_14days_for_M), ncol = 35))
# computing variances of repeated groups and rename the columns
for (i in 1:35) {
  # R: Risk
  testgroup_R <- act_14days_for_R %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  testgroup_R <- as.matrix(testgroup_R)
  variance_14days_R[,i] <- rowSds(testgroup_R)
  colnames(variance_14days_R)[i] <- paste("Rsd", combinations[3,i], sep = "_")
  # M: Mobility
  testgroup_M <- act_14days_for_M %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  testgroup_M <- as.matrix(testgroup_M)
...
# 21days
act_21days_for_R <- act_21days %>%
  select(seq(1, 3142, by = 9))
act_21days_for_M <- act_21days %>%
  select(seq(2, 3143, by = 9))

# create blank frames to store variances of variables
variance_21days_R <- data.frame(matrix(NA, nrow = nrow(act_21days_for_R), ncol = 35))
variance_21days_M <- data.frame(matrix(NA, nrow = nrow(act_21days_for_M), ncol = 35))
# computing variances of repeated groups and rename the columns
for (i in 1:35) {
  # R: Risk
  testgroup_R <- act_21days_for_R %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  testgroup_R <- as.matrix(testgroup_R)
  variance_21days_R[,i] <- rowSds(testgroup_R)
  colnames(variance_21days_R)[i] <- paste("Rsd", combinations[3,i], sep = "_")
  # M: Mobility
  testgroup_M <- act_21days_for_M %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  testgroup_M <- as.matrix(testgroup_M)
...
# 利用ggarrange将图像排列在一起
plot_variance_act <- ggarrange(plot_variance_3days_R, plot_variance_7days_R, plot_variance_14days_R, plot_variance_21days_R,
                               plot_variance_3days_M, plot_variance_7days_M, plot_variance_14days_M, plot_variance_21days_M,
                               ncol = 4,nrow =2)
plot_variance_act # 手动export大小合适的图
...
##--Discrete integral--
# act: R, M, tt(transit_traveler), ftt(failed_transit_traveler), gut(give_up_traveler), TM, rc(risk city)
#tt(flow), ftt(stock), gut(stock), rc(stock)

# 3days
# create blank frames to store means of integrals
means_integrals_3days_R <- data.frame(matrix(NA, nrow = nrow(act_3days_for_R), ncol = 70))
means_integrals_3days_M <- data.frame(matrix(NA, nrow = nrow(act_3days_for_M), ncol = 70))
means_integrals_3days_tt <- data.frame(matrix(NA, nrow = nrow(act_3days), ncol = 70))
means_3days_ftt <- data.frame(matrix(NA, nrow = nrow(act_3days), ncol = 35))
means_3days_gut <- data.frame(matrix(NA, nrow = nrow(act_3days), ncol = 35))
means_integrals_3days_TM <- data.frame(matrix(NA, nrow = nrow(act_3days), ncol = 70))
means_3days_rc <- data.frame(matrix(NA, nrow = nrow(act_3days), ncol = 35))
# computing means of repeated groups and rename the columns
act_3days_for_tt <- act_3days %>%
  select(seq(6, 3147, by = 9))
act_3days_for_ftt <- act_3days %>%
  select(seq(7, 3148, by = 9))
act_3days_for_gut <- act_3days %>%
  select(seq(8, 3149, by = 9))
act_3days_for_TM <- act_3days %>%
  select(seq(3, 3144, by = 9))
act_3days_for_rc <- act_3days %>%
...
for (i in 1:35) {
  ...
  testgroup_ftt <- act_3days_for_ftt %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
...
rm(act_3days_for_R, act_3days_for_M, act_3days_for_tt, act_3days_for_ftt, act_3days_for_gut, act_3days_for_TM, act_3days_for_rc)

# 7days
# create blank frames to store means of integrals
means_integrals_7days_R <- data.frame(matrix(NA, nrow = nrow(act_7days_for_R), ncol = 70))
means_integrals_7days_M <- data.frame(matrix(NA, nrow = nrow(act_7days_for_M), ncol = 70))
means_integrals_7days_tt <- data.frame(matrix(NA, nrow = nrow(act_7days), ncol = 70))
means_7days_ftt <- data.frame(matrix(NA, nrow = nrow(act_7days), ncol = 35))
means_7days_gut <- data.frame(matrix(NA, nrow = nrow(act_7days), ncol = 35))
means_integrals_7days_TM <- data.frame(matrix(NA, nrow = nrow(act_7days), ncol = 70))
means_7days_rc <- data.frame(matrix(NA, nrow = nrow(act_7days), ncol = 35))
# computing means of repeated groups and rename the columns
act_7days_for_tt <- act_7days %>%
  select(seq(6, 3147, by = 9))
act_7days_for_ftt <- act_7days %>%
  select(seq(7, 3148, by = 9))
act_7days_for_gut <- act_7days %>%
  select(seq(8, 3149, by = 9))
act_7days_for_TM <- act_7days %>%
  select(seq(3, 3144, by = 9))
act_7days_for_rc <- act_7days %>%
  select(seq(9, 3150, by = 9))
...
for (i in 1:35) {
  ...
  # rc: risk_city
  testgroup_rc <- act_7days_for_rc %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  means_7days_rc[,i] <- rowMeans(testgroup_rc)
  colnames(means_7days_rc)[i] <- paste("rcintegral", combinations[3,i], sep = "_")
}
rm(testgroup_R, testgroup_M, testgroup_tt, testgroup_ftt, testgroup_gut, testgroup_TM, testgroup_rc)
rm(act_7days_for_R, act_7days_for_M, act_7days_for_tt, act_7days_for_ftt, act_7days_for_gut, act_7days_for_TM, act_7days_for_rc)

# 14days
# create blank frames to store means of integrals
means_integrals_14days_R <- data.frame(matrix(NA, nrow = nrow(act_14days_for_R), ncol = 70))
means_integrals_14days_M <- data.frame(matrix(NA, nrow = nrow(act_14days_for_M), ncol = 70))
means_integrals_14days_tt <- data.frame(matrix(NA, nrow = nrow(act_14days), ncol = 70))
means_14days_ftt <- data.frame(matrix(NA, nrow = nrow(act_14days), ncol = 35))
means_14days_gut <- data.frame(matrix(NA, nrow = nrow(act_14days), ncol = 35))
means_integrals_14days_TM <- data.frame(matrix(NA, nrow = nrow(act_14days), ncol = 70))
means_14days_rc <- data.frame(matrix(NA, nrow = nrow(act_14days), ncol = 35))
# computing means of repeated groups and rename the columns
act_14days_for_tt <- act_14days %>%
  select(seq(6, 3147, by = 9))
act_14days_for_ftt <- act_14days %>%
  select(seq(7, 3148, by = 9))
act_14days_for_gut <- act_14days %>%
  select(seq(8, 3149, by = 9))
act_14days_for_TM <- act_14days %>%
  select(seq(3, 3144, by = 9))
act_14days_for_rc <- act_14days %>%
  select(seq(9, 3150, by = 9))
...
for (i in 1:35) {
  ...
  # tt: transit_traveler
  testgroup_tt <- act_14days_for_tt %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  ...
  # rc: risk_city
  testgroup_rc <- act_14days_for_rc %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  means_14days_rc[,i] <- rowMeans(testgroup_rc)
  colnames(means_14days_rc)[i] <- paste("rcintegral", combinations[3,i], sep = "_")
}
rm(testgroup_R, testgroup_M, testgroup_tt, testgroup_ftt, testgroup_gut, testgroup_TM, testgroup_rc)
rm(act_14days_for_R, act_14days_for_M, act_14days_for_tt, act_14days_for_ftt, act_14days_for_gut, act_14days_for_TM, act_14days_for_rc)

# 21days
# create blank frames to store means of integrals
means_integrals_21days_R <- data.frame(matrix(NA, nrow = nrow(act_21days_for_R), ncol = 70))
means_integrals_21days_M <- data.frame(matrix(NA, nrow = nrow(act_21days_for_M), ncol = 70))
means_integrals_21days_tt <- data.frame(matrix(NA, nrow = nrow(act_21days), ncol = 70))
means_21days_ftt <- data.frame(matrix(NA, nrow = nrow(act_21days), ncol = 35))
means_21days_gut <- data.frame(matrix(NA, nrow = nrow(act_21days), ncol = 35))
means_integrals_21days_TM <- data.frame(matrix(NA, nrow = nrow(act_21days), ncol = 70))
means_21days_rc <- data.frame(matrix(NA, nrow = nrow(act_21days), ncol = 35))
# computing means of repeated groups and rename the columns
act_21days_for_tt <- act_21days %>%
  select(seq(6, 3147, by = 9))
act_21days_for_ftt <- act_21days %>%
  select(seq(7, 3148, by = 9))
act_21days_for_gut <- act_21days %>%
  select(seq(8, 3149, by = 9))
act_21days_for_TM <- act_21days %>%
  select(seq(3, 3144, by = 9))
act_21days_for_rc <- act_21days %>%
  select(seq(9, 3150, by = 9))
...
for (i in 1:35) {
  ...
  # tt: transit_traveler
  testgroup_tt <- act_21days_for_tt %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  ...
  # rc: risk_city
  testgroup_rc <- act_21days_for_rc %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  means_21days_rc[,i] <- rowMeans(testgroup_rc)
  colnames(means_21days_rc)[i] <- paste("rcintegral", combinations[3,i], sep = "_")
}
rm(testgroup_R, testgroup_M, testgroup_tt, testgroup_ftt, testgroup_gut, testgroup_TM, testgroup_rc)
rm(act_21days_for_R, act_21days_for_M, act_21days_for_tt, act_21days_for_ftt, act_21days_for_gut, act_21days_for_TM, act_21days_for_rc)
##------------------------
...
plot_means_3days_all <- plot_for_allgroup_means %>%
  ...
  labs(
    x = "tick",
    title = "3 days + arrival + data sharing"
  )
...
plot_means_7days_all <- plot_for_allgroup_means %>%
  ...
  labs(
    x = "tick",
    title = "7 days + arrival + data sharing"
  )
...
plot_means_14days_all <- plot_for_allgroup_means %>%
  ...
  labs(
    x = "tick",
    title = "14 days + arrival + data sharing"
  )
...
plot_means_21days_all <- plot_for_allgroup_means %>%
  ...
  labs(
    x = "tick",
    title = "21 days + arrival + data sharing"
  )
...
# 利用ggarrange将图像排列在一起
plot_RM_act <- ggarrange(plot_means_3days_all, plot_means_7days_all, plot_means_14days_all, plot_means_21days_all,
...
write.xlsx(l, "arrival and data sharing-data.xlsx")
##------------------------
