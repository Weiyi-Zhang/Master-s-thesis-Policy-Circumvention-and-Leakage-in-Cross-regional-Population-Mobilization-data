#--------------------
#Objective: 毕业论文数据处理与分析-departure and data sharing
...
raw_dct <- read.csv("leakage-departure and cooperation two experiment-departure and cooperation two-spreadsheet.csv")
##------------------------

##--Formatting--
# time_option: 3, 7, 14, 21 4*350=1400
# dct
combinations <- raw_dct[c(2,3),]
...
dct <- raw_dct[c(11:nrow(raw_dct)),]
rownames(dct) <- 1:nrow(dct)
dct <- select(dct, -1)
dct[1,] <- gsub(pattern = "compute-", replacement = "", dct[1,])
dct_3days <- select(dct, 1:3150)
dct_7days <- select(dct, 3151:6300)
dct_14days <- select(dct, 6301:9450)
dct_21days <- select(dct, 9451:12600)
for (i in 1:ncol(combinations)) {
  for (j in 1:10) {
    for (w in 1:9) {
      colnames(dct_3days)[(i-1)*90+(j-1)*9+w] <- paste(dct_3days[1,(i-1)*90+(j-1)*9+w], j, combinations[3,i], sep = "_")
      colnames(dct_7days)[(i-1)*90+(j-1)*9+w] <- paste(dct_7days[1,(i-1)*90+(j-1)*9+w], j, combinations[3,i], sep = "_")
      colnames(dct_14days)[(i-1)*90+(j-1)*9+w] <- paste(dct_14days[1,(i-1)*90+(j-1)*9+w], j, combinations[3,i], sep = "_")
      colnames(dct_21days)[(i-1)*90+(j-1)*9+w] <- paste(dct_21days[1,(i-1)*90+(j-1)*9+w], j, combinations[3,i], sep = "_")
    }
  }
}
dct_3days <- dct_3days[-c(1,2),]
dct_7days <- dct_7days[-c(1,2),]
dct_14days <- dct_14days[-c(1,2),]
dct_21days <- dct_21days[-c(1,2),]
rownames(dct_3days) <- 1:nrow(dct_3days)
rownames(dct_7days) <- 1:nrow(dct_7days)
rownames(dct_14days) <- 1:nrow(dct_14days)
rownames(dct_21days) <- 1:nrow(dct_21days)
for (i in 1:ncol(dct_3days)) {
  dct_3days[,i] <- as.numeric(dct_3days[,i])
  dct_7days[,i] <- as.numeric(dct_7days[,i])
  dct_14days[,i] <- as.numeric(dct_14days[,i])
  dct_21days[,i] <- as.numeric(dct_21days[,i])
}
##------------------------
...
# dct: R, M
# 3days
dct_3days_for_R <- dct_3days %>%
  select(seq(1, 3142, by = 9))
dct_3days_for_M <- dct_3days %>%
  select(seq(2, 3143, by = 9))

# create blank frames to store variances of variables
variance_3days_R <- data.frame(matrix(NA, nrow = nrow(dct_3days_for_R), ncol = 35))
variance_3days_M <- data.frame(matrix(NA, nrow = nrow(dct_3days_for_M), ncol = 35))
# computing variances of repeated groups and rename the columns
for (i in 1:35) {
  # R: Risk
  testgroup_R <- dct_3days_for_R %>%
...
# 7days
dct_7days_for_R <- dct_7days %>%
  select(seq(1, 3142, by = 9))
dct_7days_for_M <- dct_7days %>%
  select(seq(2, 3143, by = 9))

# create blank frames to store variances of variables
variance_7days_R <- data.frame(matrix(NA, nrow = nrow(dct_7days_for_R), ncol = 35))
variance_7days_M <- data.frame(matrix(NA, nrow = nrow(dct_7days_for_M), ncol = 35))
# computing variances of repeated groups and rename the columns
for (i in 1:35) {
  # R: Risk
  testgroup_R <- dct_7days_for_R %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  testgroup_R <- as.matrix(testgroup_R)
  variance_7days_R[,i] <- rowSds(testgroup_R)
  colnames(variance_7days_R)[i] <- paste("Rsd", combinations[3,i], sep = "_")
  # M: Mobility
  testgroup_M <- dct_7days_for_M %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  testgroup_M <- as.matrix(testgroup_M)
...
# 14days
dct_14days_for_R <- dct_14days %>%
  select(seq(1, 3142, by = 9))
dct_14days_for_M <- dct_14days %>%
  select(seq(2, 3143, by = 9))

# create blank frames to store variances of variables
variance_14days_R <- data.frame(matrix(NA, nrow = nrow(dct_14days_for_R), ncol = 35))
variance_14days_M <- data.frame(matrix(NA, nrow = nrow(dct_14days_for_M), ncol = 35))
# computing variances of repeated groups and rename the columns
for (i in 1:35) {
  # R: Risk
  testgroup_R <- dct_14days_for_R %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  testgroup_R <- as.matrix(testgroup_R)
  variance_14days_R[,i] <- rowSds(testgroup_R)
  colnames(variance_14days_R)[i] <- paste("Rsd", combinations[3,i], sep = "_")
  # M: Mobility
  testgroup_M <- dct_14days_for_M %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  testgroup_M <- as.matrix(testgroup_M)
...
# 21days
dct_21days_for_R <- dct_21days %>%
  select(seq(1, 3142, by = 9))
dct_21days_for_M <- dct_21days %>%
  select(seq(2, 3143, by = 9))

# create blank frames to store variances of variables
variance_21days_R <- data.frame(matrix(NA, nrow = nrow(dct_21days_for_R), ncol = 35))
variance_21days_M <- data.frame(matrix(NA, nrow = nrow(dct_21days_for_M), ncol = 35))
# computing variances of repeated groups and rename the columns
for (i in 1:35) {
  # R: Risk
  testgroup_R <- dct_21days_for_R %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  testgroup_R <- as.matrix(testgroup_R)
  variance_21days_R[,i] <- rowSds(testgroup_R)
  colnames(variance_21days_R)[i] <- paste("Rsd", combinations[3,i], sep = "_")
  # M: Mobility
  testgroup_M <- dct_21days_for_M %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  testgroup_M <- as.matrix(testgroup_M)
...
# 利用ggarrange将图像排列在一起
plot_variance_dct <- ggarrange(plot_variance_3days_R, plot_variance_7days_R, plot_variance_14days_R, plot_variance_21days_R,
                               plot_variance_3days_M, plot_variance_7days_M, plot_variance_14days_M, plot_variance_21days_M,
                               ncol = 4,nrow =2)
plot_variance_dct # 手动export大小合适的图
...
##--Discrete integral--
# dct: R, M, tt(transit_traveler), ftt(failed_transit_traveler), gut(give_up_traveler), TM, rc(risk city)
#tt(flow), ftt(stock), gut(stock), rc(stock)

# 3days
# create blank frames to store means of repeated simulations
means_integrals_3days_R <- data.frame(matrix(NA, nrow = nrow(dct_3days_for_R), ncol = 70))
means_integrals_3days_M <- data.frame(matrix(NA, nrow = nrow(dct_3days_for_M), ncol = 70))
means_integrals_3days_tt <- data.frame(matrix(NA, nrow = nrow(dct_3days), ncol = 70))
means_3days_ftt <- data.frame(matrix(NA, nrow = nrow(dct_3days), ncol = 35))
means_3days_gut <- data.frame(matrix(NA, nrow = nrow(dct_3days), ncol = 35))
means_integrals_3days_TM <- data.frame(matrix(NA, nrow = nrow(dct_3days), ncol = 70))
means_3days_rc <- data.frame(matrix(NA, nrow = nrow(dct_3days), ncol = 35))
# computing means of repeated groups and rename the columns
dct_3days_for_tt <- dct_3days %>%
  select(seq(6, 3147, by = 9))
dct_3days_for_ftt <- dct_3days %>%
  select(seq(7, 3148, by = 9))
dct_3days_for_gut <- dct_3days %>%
  select(seq(8, 3149, by = 9))
dct_3days_for_TM <- dct_3days %>%
  select(seq(3, 3144, by = 9))
dct_3days_for_rc <- dct_3days %>%
...
for (i in 1:35) {
  ...
  testgroup_ftt <- dct_3days_for_ftt %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
...
rm(dct_3days_for_R, dct_3days_for_M, dct_3days_for_tt, dct_3days_for_ftt, dct_3days_for_gut, dct_3days_for_TM, dct_3days_for_rc)

# 7days
# create blank frames to store means of repeated simulations
means_integrals_7days_R <- data.frame(matrix(NA, nrow = nrow(dct_7days_for_R), ncol = 70))
means_integrals_7days_M <- data.frame(matrix(NA, nrow = nrow(dct_7days_for_M), ncol = 70))
means_integrals_7days_tt <- data.frame(matrix(NA, nrow = nrow(dct_7days), ncol = 70))
means_7days_ftt <- data.frame(matrix(NA, nrow = nrow(dct_7days), ncol = 35))
means_7days_gut <- data.frame(matrix(NA, nrow = nrow(dct_7days), ncol = 35))
means_integrals_7days_TM <- data.frame(matrix(NA, nrow = nrow(dct_7days), ncol = 70))
means_7days_rc <- data.frame(matrix(NA, nrow = nrow(dct_7days), ncol = 35))
# computing means of repeated groups and rename the columns
dct_7days_for_tt <- dct_7days %>%
  select(seq(6, 3147, by = 9))
dct_7days_for_ftt <- dct_7days %>%
  select(seq(7, 3148, by = 9))
dct_7days_for_gut <- dct_7days %>%
  select(seq(8, 3149, by = 9))
dct_7days_for_TM <- dct_7days %>%
  select(seq(3, 3144, by = 9))
dct_7days_for_rc <- dct_7days %>%
  select(seq(9, 3150, by = 9))
...
for (i in 1:35) {
  ...
  # rc: risk_city
  testgroup_rc <- dct_7days_for_rc %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  means_7days_rc[,i] <- rowMeans(testgroup_rc)
  colnames(means_7days_rc)[i] <- paste("rcintegral", combinations[3,i], sep = "_")
}
rm(testgroup_R, testgroup_M, testgroup_tt, testgroup_ftt, testgroup_gut, testgroup_TM, testgroup_rc)
rm(dct_7days_for_R, dct_7days_for_M, dct_7days_for_tt, dct_7days_for_ftt, dct_7days_for_gut, dct_7days_for_TM, dct_7days_for_rc)

# 14days
# create blank frames to store means of repeated simulations
means_integrals_14days_R <- data.frame(matrix(NA, nrow = nrow(dct_14days_for_R), ncol = 70))
means_integrals_14days_M <- data.frame(matrix(NA, nrow = nrow(dct_14days_for_M), ncol = 70))
means_integrals_14days_tt <- data.frame(matrix(NA, nrow = nrow(dct_14days), ncol = 70))
means_14days_ftt <- data.frame(matrix(NA, nrow = nrow(dct_14days), ncol = 35))
means_14days_gut <- data.frame(matrix(NA, nrow = nrow(dct_14days), ncol = 35))
means_integrals_14days_TM <- data.frame(matrix(NA, nrow = nrow(dct_14days), ncol = 70))
means_14days_rc <- data.frame(matrix(NA, nrow = nrow(dct_14days), ncol = 35))
# computing means of repeated groups and rename the columns
dct_14days_for_tt <- dct_14days %>%
  select(seq(6, 3147, by = 9))
dct_14days_for_ftt <- dct_14days %>%
  select(seq(7, 3148, by = 9))
dct_14days_for_gut <- dct_14days %>%
  select(seq(8, 3149, by = 9))
dct_14days_for_TM <- dct_14days %>%
  select(seq(3, 3144, by = 9))
dct_14days_for_rc <- dct_14days %>%
  select(seq(9, 3150, by = 9))
...
for (i in 1:35) {
  ...
  # tt: transit_traveler
  testgroup_tt <- dct_14days_for_tt %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  ...
  # rc: risk_city
  testgroup_rc <- dct_14days_for_rc %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  means_14days_rc[,i] <- rowMeans(testgroup_rc)
  colnames(means_14days_rc)[i] <- paste("rcintegral", combinations[3,i], sep = "_")
}
rm(testgroup_R, testgroup_M, testgroup_tt, testgroup_ftt, testgroup_gut, testgroup_TM, testgroup_rc)
rm(dct_14days_for_R, dct_14days_for_M, dct_14days_for_tt, dct_14days_for_ftt, dct_14days_for_gut, dct_14days_for_TM, dct_14days_for_rc)

# 21days
# create blank frames to store means of repeated simulations
means_integrals_21days_R <- data.frame(matrix(NA, nrow = nrow(dct_21days_for_R), ncol = 70))
means_integrals_21days_M <- data.frame(matrix(NA, nrow = nrow(dct_21days_for_M), ncol = 70))
means_integrals_21days_tt <- data.frame(matrix(NA, nrow = nrow(dct_21days), ncol = 70))
means_21days_ftt <- data.frame(matrix(NA, nrow = nrow(dct_21days), ncol = 35))
means_21days_gut <- data.frame(matrix(NA, nrow = nrow(dct_21days), ncol = 35))
means_integrals_21days_TM <- data.frame(matrix(NA, nrow = nrow(dct_21days), ncol = 70))
means_21days_rc <- data.frame(matrix(NA, nrow = nrow(dct_21days), ncol = 35))
# computing means of repeated groups and rename the columns
dct_21days_for_tt <- dct_21days %>%
  select(seq(6, 3147, by = 9))
dct_21days_for_ftt <- dct_21days %>%
  select(seq(7, 3148, by = 9))
dct_21days_for_gut <- dct_21days %>%
  select(seq(8, 3149, by = 9))
dct_21days_for_TM <- dct_21days %>%
  select(seq(3, 3144, by = 9))
dct_21days_for_rc <- dct_21days %>%
  select(seq(9, 3150, by = 9))
...
for (i in 1:35) {
  ...
  # tt: transit_traveler
  testgroup_tt <- dct_21days_for_tt %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  ...
  # rc: risk_city
  testgroup_rc <- dct_21days_for_rc %>%
    select(seq(from = (i-1)*10+1, to = i*10, by = 1))
  means_21days_rc[,i] <- rowMeans(testgroup_rc)
  colnames(means_21days_rc)[i] <- paste("rcintegral", combinations[3,i], sep = "_")
}
rm(testgroup_R, testgroup_M, testgroup_tt, testgroup_ftt, testgroup_gut, testgroup_TM, testgroup_rc)
rm(dct_21days_for_R, dct_21days_for_M, dct_21days_for_tt, dct_21days_for_ftt, dct_21days_for_gut, dct_21days_for_TM, dct_21days_for_rc)
##------------------------
...
plot_means_3days_all <- plot_for_allgroup_means %>%
  ...
  labs(
    x = "tick",
    title = "3 days + departure + data sharing"
  )
...
plot_means_7days_all <- plot_for_allgroup_means %>%
  ...
  labs(
    x = "tick",
    title = "7 days + departure + data sharing"
  )
...
plot_means_14days_all <- plot_for_allgroup_means %>%
  ...
  labs(
    x = "tick",
    title = "14 days + departure + data sharing"
  )
...
plot_means_21days_all <- plot_for_allgroup_means %>%
  ...
  labs(
    x = "tick",
    title = "21 days + departure + data sharing"
  )
...
# 利用ggarrange将图像排列在一起
plot_RM_dct <- ggarrange(plot_means_3days_all, plot_means_7days_all, plot_means_14days_all, plot_means_21days_all,
...
write.xlsx(l, "departure and data sharing-data.xlsx")
##------------------------
